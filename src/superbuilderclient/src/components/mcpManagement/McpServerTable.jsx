...
export default function McpServerTable({ layoutMode = 'vertical' }) {
  ...
  const mcpServers = React.useMemo(
    () => generateUniqueRows(rawMcpServers, "name"),
    [rawMcpServers]
  );
      ...
      return (
        ...
        >
          <Button
            ...
            disabled={
              !isChatReady ||
              loadingMcpServers.includes(params.row.name) ||
              runningMcpServers.includes(params.row.name)
            }
            sx={{ marginLeft: 1 }}
            onClick={() => handleDetailsClick(params.row.id)}
            data-testid={`server-table-edit-btn-${params.row.name}`}
          >
  ...
  const handleSelectionChange = selectionModel => {
    ...
    console.debug('Selected MCP IDs:', selectedIds);
    useMcpStore.getState().setSelectedMcpServerNames(selectedIds);

    const selectedServers = mcpServers.filter(server => selectedIds.includes(server.id));
  ...
  const handleDetailsClick = id => {
    ...
    useMcpStore.getState().openMcpInput('Update', selectedServer.url === '' ? 'command' : 'url');
    useMcpStore.getState().setMcpInput({
      mcpServerName: selectedServer.name || '',
    ...
    if (runningMcpServers.includes(selectedServer.name)) {
      useMcpStore.getState().getMcpServerTools(selectedServer.name);
    }
  };
  ...
  const commandColumns = React.useMemo(
    () => [
      createTextColumn('name', t('mcp.server_table.name'), 0.5, 120),
  ...
  return (
    <Box sx={MCP_TABLE_STYLES.container} data-testid="mcp-servers-table-container">
      <Paper sx={paperStyle}>
        <DataGrid
          rows={mcpServers}
